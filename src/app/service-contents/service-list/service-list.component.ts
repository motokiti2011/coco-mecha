...
export class ServiceListComponent implements OnInit {
   ...
   * @return void
   */
  private initSetServiceContents(slipDetail: slipDetail[]): void {
    // 初回のみ初期化
   ...
   * @param slipDetail 
   */
  private seviceListSetting(slipDetail: slipDetail[]): void {
    slipDetail.forEach((content) => {
      if (!(_isNil(content))) {
   ...
   * @return void
   */
  private setServiceContents(slipDetail: slipDetail[]): void {
    this.displayContentsList = [];
    // コンテンツ全体から総ページ数を算出する
    this.totalPage = Math.round(slipDetail.length / 6);
    // 現在のページを設定する
